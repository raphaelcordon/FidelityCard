...
namespace AzureBlobStorage.Repository
{
	public class AzureStorage : IAzureStorage
	{
		...
		public async Task<BlobResponseDto> UploadAsync(IFormFile blob)
		{
			...
			{
				_logger.LogError($"File with name {blob.FileName} already exists. Choose another name");
				response.Status = $"File with name {blob.FileName} already exists. Choose another name";
				response.Error = true;
				return response;
			}
			catch (Exception ex)
...
/*
public async Task UploadDocument(string fileName, Stream fileContent)
{
	var container = GetContainer();
	if (!await container.ExistsAsync())
	{
		...
		container = blobServiceClient.GetBlobContainerClient(_containerName);
	}
